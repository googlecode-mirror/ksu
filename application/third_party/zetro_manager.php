...
class zetro_manager{
   ...
   function parse_ini_file($filename){
            ...
            if ( is_array( $INIarray ) )
            {
                ...
                { 
                    if ( is_array( $a ) )
                    {
                        foreach ($a as $n => $value)
                        { 
                            if ( strcmp( $i, base64_encode($keyNAME) ) == 0 && 
							   strcmp( $n, base64_encode($entryNAME) ) == 0 )
                            { 
                                // don't do anything !
							} else $this->bi_a[$i][$n] = $a[$n] ;
							echo strcmp( $i, base64_encode($keyNAME) );//echo"akan di hapus";
                        }
                    }
                }
            }
	...
	function Count($keyName,$path){
	 ...
	 {
		 $entry_val=$this->get_content($path,base64_encode($keyName),base64_encode($entryName));
		 return $entry_val;
	 }
	 /////////////////////////////////////////////////////////
